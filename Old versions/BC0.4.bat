...
chcp 65001
cls
set program_version=beta 0.0.0.4
title BATCH COMPILER %program_version%
...
if "%sentence%"=="" goto total

:skip_goto_total
echo skip goto total
set old_sentence=%sentence%
rem for %%i in (check.tmp) do @set count=%%~zi
:str_check
...
if "!sentence:~3,1!"=="!quotes!" (echo variabile trovata nel primo valore
goto trova_valore1)
...
if "!sentence:~%space%,%long%!"=="goto" (echo comando goto trovato
echo %sentence%; >>%output%)
:skip_goto
